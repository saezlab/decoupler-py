...
def decouple(mat, net, source='source', target='target', weight='weight',
             methods=None, args={}, consensus=True, cns_metds=None, 
             min_n=5, verbose=True, use_raw=True):
    """
    Decouple function.
    ...
    methods : list, str
        List of methods to run. If none are provided use weighted top performers 
        (mlm, ulm and wsum). To run all methods set to "all".
    args : dict
        A dict of argument-dicts.
    consensus : bool
        Boolean whether to run a consensus score between methods.
    cns_metds : list
        List of estimate names to use for the calculation of the consensus score. If 
        empty it will use all the estimates obtained after running the different methods.
    min_n : int
