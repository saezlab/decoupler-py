...
from ..utils_anndata import get_acts, extract_psbulk_inputs, check_for_raw_counts, format_psbulk_inputs, compute_psbulk, get_unq_dict, get_pseudobulk, check_if_skip, get_contrast, get_top_targets, format_contrast_results
...
def test_compute_psbulk():
    ...
    compute_psbulk(psbulk, props, X, sample_col, groups_col, np.unique(smples), np.unique(groups), obs, new_obs, min_cells, min_counts, min_prop)
    compute_psbulk(psbulk, props, X, sample_col, None, np.unique(smples), np.unique(groups), obs, new_obs, min_cells, min_counts, min_prop)
